        ...
        for attempt in range(max_retries):
            try:
                async with httpx.AsyncClient(verify=False, headers=headers, timeout=15.0) as client:
                    ...
                    if response.status_code == 403:
                        if attempt < max_retries - 1:
                            # 指数バックオフでリトライ
                            wait_time = 2 ** attempt
                            print(f"403エラー発生。{wait_time}秒待機してリトライ... (試行 {attempt + 1}/{max_retries})")
                    ...
                    if not content or len(content) < 50:
                        if attempt < max_retries - 1:
                            wait_time = 2 ** attempt
                            print(f"コンテンツが取得できませんでした。{wait_time}秒待機してリトライ... (試行 {attempt + 1}/{max_retries})")
                            await asyncio.sleep(wait_time)
                            continue
                        return None
                ...
                if e.response.status_code == 403 and attempt < max_retries - 1:
                    wait_time = 2 ** attempt
                    print(f"HTTPステータスエラー: {e.response.status_code}。{wait_time}秒待機してリトライ...")
                    await asyncio.sleep(wait_time)
                    continue
                raise
                ...
                if attempt < max_retries - 1:
                    wait_time = 2 ** attempt
                    print(f"エラー発生: {str(e)}。{wait_time}秒待機してリトライ... (試行 {attempt + 1}/{max_retries})")
                    await asyncio.sleep(wait_time)
                    continue
